...
const Section = (opt: TSection) => {
	...
	return (
		...
		<div
			...
			{/* CONTENIDO */}
			<div
				// className={`${className} text-main flex w-full max-w-section-content flex-col items-center justify-center border-2 border-blue-700 p-4 sm:p-8 ${className}`}>
				className={`${className} text-main flex w-full max-w-section-content flex-col items-center justify-center p-4 sm:p-8 ${className}`}>
				{children}
			</div>
		</div>
	)
